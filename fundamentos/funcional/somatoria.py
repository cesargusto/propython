...
if somatoria is soma_rec:
    ...
    assert abs(pi - soma_pi(1, 1000000)) < PRECISAO

assert abs(pi - sum(1.0 / (x * (x + 2)) for x in xrange(1, 1000000, 4)) * 8)  < PRECISAO

def integral(f, a, b, dx):
    def inc_dx(x): return x + dx
    return somatoria(f, a + dx/2.0, inc_dx, b) * dx
    
assert abs(.25 - integral(cubo, 0, 1, 0.001)) < PRECISAO
assert abs((1.0/3) - integral(lambda x:x*x, 0, 1, 0.01)) < PRECISAO
assert abs((2.0/3) - integral(sqrt, 0, 1, 0.001)) < PRECISAO
assert abs(18 - integral(sqrt, 0, 9, 0.001)) < PRECISAO
...
if somatoria is soma_rec:
    ...
    assert abs(in_pot(7)(2)-integral(lambda x:x**7, 0, 2, 0.0001)) < PRECISAO
